...
public class IRNodeTranslator implements NodeTranslator {
                ...
                nodeAugmentationBuilder.withNodeIdentifier(NODE_AUGMENTATION_IDENTIFIER)
                        .withChild(ImmutableContainerNodeBuilder.create()
                                ...
                                .withChild(ImmutableNodes.leafNode(IRQNames.NODE_AUG_SOFTWARE_QNAME,
                                        NormalizedNodes.findNode(inventoryNodeAugNode.get(), IRInstanceIdentifiers
                                                .OPEN_FLOW_NODE_SOFTWARE_IDENTIFIER).get().getValue()))
                                .build());
            } catch (IllegalStateException e) {
                LOG.error("One or more fields in inventory node are not present", e);
                throw e;
            }
        }
    ...
    private MapEntryNode createTerminationPoint(MapEntryNode nodeConnectorEntry,
        ...
        try {
            String ncId = nodeConnectorEntry.getChild(
                    YangInstanceIdentifier.of(IRQNames.NODE_CONNECTOR_ID_QNAME)
                            .getLastPathArgument()).get().getValue().toString();
            terminationPointId = terminationPointsIds.remove(ncId);
            if (terminationPointId == null) {
                return null;
            }
        } catch (IllegalStateException e) {
            LOG.error("Node connector id is not present", e);
            throw e;
        }
        ...
        if (inventoryNodeConnectorAugNode.isPresent()) {
            try {
                ...
                LOG.error("One or more fields in inventory node connector are not present", e);
                throw e;
            }
        }
    ...
    private Map<String, String> getTerminationPointsIds(NormalizedNode<?, ?> terminationPointsNode) {
        ...
        for (MapEntryNode terminationPointEntry : terminationPointsEntries) {
            ...
            String tpRef = NormalizedNodes.findNode(terminationPointEntry,
                    IRInstanceIdentifiers.TP_REF_IDENTIFIER).get().getValue().toString();
