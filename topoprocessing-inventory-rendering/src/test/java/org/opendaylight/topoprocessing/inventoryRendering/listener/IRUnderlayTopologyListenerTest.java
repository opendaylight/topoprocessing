...
public class IRUnderlayTopologyListenerTest {
    ...
    public void testAggregation() {
        ...
        UnderlayTopologyListener listener = new IRUnderlayTopologyListener(domDataBroker, TOPOLOGY_ID,
        ...
        UnderlayItem physicalNode = new UnderlayItem(nodeValueWithIp, nodeIpValue, TOPOLOGY_ID, nodeName,
                CorrelationItemEnum.Node);
    ...
    public void testReadExistingData() {
        ...
        TopologyOperator mockOperator = Mockito.mock(TopologyOperator.class);
        YangInstanceIdentifier path = YangInstanceIdentifier.of(TopologyQNames.NETWORK_NODE_ID_QNAME);
        UnderlayTopologyListener listener = new IRUnderlayTopologyListener(domDataBrokerLocal,
                TOPOLOGY_ID, CorrelationItemEnum.Node);
        listener.setOperator(mockOperator);
        DOMDataReadOnlyTransaction readTransaction = Mockito.mock(DOMDataReadOnlyTransaction.class);
        Mockito.when(domDataBrokerLocal.newReadOnlyTransaction()).thenReturn(readTransaction);
        Optional<NormalizedNode<?,?>> mockOptional = Mockito.mock(Optional.class);
        CheckedFuture<Optional<NormalizedNode<?, ?>>, ReadFailedException> checkedFuture
                = Futures.immediateCheckedFuture(mockOptional);
        Mockito.when(readTransaction.read(LogicalDatastoreType.OPERATIONAL, path)).thenReturn(checkedFuture);
        MapEntryNode nodeNode = ImmutableNodes.mapEntryBuilder(Node.QNAME,
                TopologyQNames.NETWORK_NODE_ID_QNAME, "node:1")
                .addChild(ImmutableNodes.leafNode(TopologyQNames.NETWORK_NODE_ID_QNAME, "node:1"))
                .build();
        final NormalizedNode<?,?> mapNode = ImmutableNodes.mapNodeBuilder(Node.QNAME)
                .addChild(nodeNode)
                .build();
        Answer<NormalizedNode<?,?>> answer = new Answer<NormalizedNode<?,?>>() {
            @Override
            public NormalizedNode<?,?> answer(InvocationOnMock invocation)
                    throws Throwable {
                return mapNode;
            }
        };
        Mockito.when(mockOptional.isPresent()).thenReturn(true);
        Mockito.when(mockOptional.get()).then(answer);
        Map<YangInstanceIdentifier, UnderlayItem> resultEntries = new HashMap<>();
        resultEntries.put(YangInstanceIdentifier
                .builder(InstanceIdentifiers.TOPOLOGY_IDENTIFIER)
                .nodeWithKey(Topology.QNAME, TopologyQNames.TOPOLOGY_ID_QNAME, TOPOLOGY_ID)
                .node(Node.QNAME)
                .nodeWithKey(Node.QNAME, TopologyQNames.NETWORK_NODE_ID_QNAME, "node:1")
                .build(),
                new UnderlayItem(nodeNode, null, TOPOLOGY_ID, "node:1",
                        CorrelationItemEnum.Node));

        listener.readExistingData(path, DatastoreType.OPERATIONAL);
        Mockito.verify(mockOperator).processCreatedChanges(Matchers.refEq(resultEntries), Matchers.eq(TOPOLOGY_ID));
    }
}
