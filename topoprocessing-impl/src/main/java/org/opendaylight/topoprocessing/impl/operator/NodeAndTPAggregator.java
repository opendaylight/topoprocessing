...
public class NodeAndTPAggregator implements TopologyOperator {
    ...
    /*  This class consists of node aggregator and termination point aggregator. In aggregators are custom managers.
     *  Flow after changes are made:
     *      1. Node aggregation using node aggregator
     *      2. After aggregation of nodes is in node aggregator called manager (nodeManager) to write OverlayItem
     *      3. nodeManager calls method addTPsToWrapper
     *      4. Method addTPsToWrapper creates "fake" node with all termination points from wrapper
     *      5. Termination points in "fake" node are aggregated in termination point aggregator
     *      6. After aggregation of tps is in tp aggregator called manager (tpManager)
     *      7. From "fake" node are extracted aggregated termination points and inserted in wrapper
     */
                    ...
                    if(tpMapNodeOpt.isPresent()) {
                        ...
                        for (MapEntryNode tpMapEntryNode : tpMapNode.getValue()) {
                            ...
                            if(tpIdOpt.isPresent()) {
                                ...
                                // create unique id
                                String id = underlayItemFromWrapper.getTopologyId() + "/"
                                                + underlayItemFromWrapper.getItemId() + "/"
                                                + tpIdOpt.get().getValue().toString();
                                targetFieldsPerTP.put(id, targetFields);
                                terminationPoints.add(ImmutableNodes.mapEntryBuilder(tpQName, tpIdQname, id)
                                                .withValue(value).build());
                            } else {
                                LOG.debug("Termination point ID is not present!");
                            }
                        }
                    } else {
                        LOG.debug("Termination point MapNode is not present!");
                    }
                }
            }
        }

        String nodeId = wrapperId + "";
