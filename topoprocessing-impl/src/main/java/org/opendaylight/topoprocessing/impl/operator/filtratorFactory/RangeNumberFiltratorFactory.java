...
public class RangeNumberFiltratorFactory implements FiltratorFactory {
    ...
    public Filtrator createFiltrator(Filter filter, YangInstanceIdentifier pathIdentifier) {
        return new RangeNumber(filter.getAugmentation(RangeNumberFiltrator.class).getMinNumberValue(),
                filter.getAugmentation(RangeNumberFiltrator.class).getMaxNumberValue(), pathIdentifier);
    }
}
