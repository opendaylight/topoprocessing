        ...
        for (LogicalNode logicalNode : wrapper.getLogicalNodes()) {
            ...
            for (PhysicalNode physicalNode : logicalNode.getPhysicalNodes()) {
                if (! writtenNodes.contains(physicalNode)) {
                    ...
                    keyValues.put(TopologyQNames.TOPOLOGY_REF, YangInstanceIdentifier.create(Iterables.limit(
                            physicalNode.getNodeIdentifier().getPathArguments(), 3)).toString());
                    keyValues.put(TopologyQNames.NODE_REF, physicalNode.getNodeIdentifier().toString());
