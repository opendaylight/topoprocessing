...
public class TerminationPointAggregator extends UnificationAggregator {
    ...
    private class TemporaryTerminationPoint {
        ...
        public MapEntryNode getById(YangInstanceIdentifier.NodeIdentifierWithPredicates nodeIdentifier) {
        ...
        public void setTargetField(NormalizedNode<?, ?> targetField) {
...
//                // look for TP MapNode in Updated Node
    ...
    private void addItemToList(MapEntryNode tpMapEntry, NormalizedNode<?, ?> targetField,
        ...
        tmpTp.setTargetField(targetField);
        ...
        for (MapEntryNode newTpEntry : newTpMap.getValue()) {
            ...
            if (targetFieldOpt.isPresent()) {
                ...
                for (TemporaryTerminationPoint tmpTp : tempTpList) {
                    MapEntryNode oldTpEntry = tmpTp.getById(newTpEntry.getIdentifier());
                    // check if node with same ID exists in TP-store
                    if (null != oldTpEntry) {
                        // if nodes are equal
                        if (oldTpEntry.equals(newTpEntry)) {
                            isNew = false;
                        } else {
                            // remove it
                ...
                if (isNew) {
                    TemporaryTerminationPoint ttp = new TemporaryTerminationPoint();
                    ttp.setTargetField(targetFieldOpt.get());
