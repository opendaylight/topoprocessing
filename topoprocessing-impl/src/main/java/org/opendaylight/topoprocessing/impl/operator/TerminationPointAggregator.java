...
public class TerminationPointAggregator extends UnificationAggregator {
    ...
    private class TemporaryTerminationPoint extends ArrayList<MapEntryNode> {}
    private class TemporaryTerminationPointMap {
        private Map<Object, TemporaryTerminationPoint> map = new HashMap<>();

        public Map<Object, TemporaryTerminationPoint> getMap() {
            return map;
        }

        public void add(MapEntryNode tp) {
            Optional<NormalizedNode<?, ?>> targetField = NormalizedNodes.findNode(tp, leafPath);
            if (targetField.isPresent()) {
                LeafNode key = (LeafNode) targetField.get();
                if (! map.containsKey(key)) {
                    map.put(key, new TemporaryTerminationPoint());
                }
                map.get(key).add(tp);
            }
        }
    }
    private Map<YangInstanceIdentifier, TemporaryTerminationPointMap> tpStore = new HashMap<>();
    ...
    public void processCreatedChanges(Map<YangInstanceIdentifier, UnderlayItem> createdEntries,
        ...
        if (createdEntries != null) {
            ...
            for (Map.Entry<YangInstanceIdentifier, UnderlayItem> createdEntry : createdEntries.entrySet()) {
                ...
                TemporaryTerminationPointMap tempTpMap = tpStore.get(createdEntry.getKey());
                ...
                if (tpMapNode.isPresent()) {
                    ...
                    for (MapEntryNode mapEntryNode : initTpMapNode.getValue()) {
                        tempTpMap.add(mapEntryNode);
                    }
                    //TODO process map
    ...
    public void processUpdatedChanges(Map<YangInstanceIdentifier, UnderlayItem> updatedEntries, String topologyId) {
        ...
        if (updatedEntries != null) {
            ...
            for (Map.Entry<YangInstanceIdentifier, UnderlayItem> updatedEntry : updatedEntries.entrySet()) {
                ...
                MapEntryNode node = (MapEntryNode) updatedEntry.getValue().getItem();
                TemporaryTerminationPointMap tempTpMap = tpStore.get(updatedEntry.getKey());
                ...
                if (updatedTpMapNode.isPresent()
                    ...
                    MapEntryNode newTpMap = (MapEntryNode) updatedTpMapNode.get();
                    for (DataContainerChild<? extends YangInstanceIdentifier.PathArgument, ?> newTpEntry
                            : newTpMap.getValue()) {
...
//                for (Map.Entry<Object, TemporaryTerminationPoint> tempTp : tempTpMap.getMap().entrySet()) {
//                    for (MapEntryNode tp : tempTp.getValue()) {
//                        Optional<NormalizedNode<?, ?>> tpIdOpt = NormalizedNodes.findNode(
//                                tp, YangInstanceIdentifier.of(TopologyQNames.NETWORK_TP_ID_QNAME));
//                        if (tpIdOpt.isPresent() && tpIdOpt.get().equals()) {
//
//                        }
//                    }
//                }
//                NormalizedNode<?, ?> oldTPs = NormalizedNodes.findNode(underlayItem.getItem(),
