...
public abstract class TopologyRequestHandler {
    ...
    private void initAggregationOfNodesAndTp(Correlation nodeCorrelation, boolean nodeFiltration,
        ...
        TopologyAggregator nodeAggregator = createAggregator(nodeAggregation.getAggregationType(), CorrelationItemEnum.Node, topoStoreProvider,
                        null);
        TopologyAggregator tpAggregator = createAggregator(tpAggregation.getAggregationType(), CorrelationItemEnum.TerminationPoint, topoStoreProvider,
        ...
        for (Mapping mapping : tpAggregation.getMapping()) {
            inputModel = getRealInputModel(mapping.getInputModel(), CorrelationItemEnum.TerminationPoint);
            ...
            if (tpFiltration && mapping.getApplyFilters() != null) {
                ...
                filtrator.setTopologyAggregator(tpAggregator);
        ...
        } // TODO - POZRI FILTRACIU - treba kdesi filtrovat TP predtym, nez sa zagreguju


        // todo pridat filtrator do konstruktoru
        NodeAndTPAggregator nodeAndTpAggregator = new NodeAndTPAggregator(nodeAggregator, tpAggregator, tpAggregation.getMapping().get(0).getInputModel(), targetFieldsPerTopology);
        ...
        for (Mapping mapping : nodeAggregation.getMapping()) {
            ...
            if (filtrator == null) {
                listener = modelAdapters.get(inputModel)
                        .registerUnderlayTopologyListener(pingPongDataBroker, underlayTopologyId, CorrelationItemEnum.Node,
                                datastoreType, nodeAndTpAggregator, listeners, pathIdentifier);
            } else {
                listener = modelAdapters.get(inputModel)
                        .registerUnderlayTopologyListener(pingPongDataBroker, underlayTopologyId, CorrelationItemEnum.Node,
