...
public class TopologyRequestHandler {
    ...
    private static final int MAX_TEAR_DOWN_WAIT_TIME = 1000;
...
private Filter findFilter(List<Filter> filters, String filterId) {
    ...
    private void closeOperatingResources() {
        ...
        listeners.clear();
        Future<Boolean> tearDownFuture = writer.tearDown();
        try {
            tearDownFuture.get(MAX_TEAR_DOWN_WAIT_TIME, TimeUnit.MILLISECONDS);
        } catch (InterruptedException | ExecutionException | TimeoutException e) {
            LOG.error("An error occurred while waiting for tear down future: {}", e);
        }
