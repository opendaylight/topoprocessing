...
public class TopologyRequestHandler {
    ...
    private void initializeListener(final String underlayTopologyId, DatastoreType datastoreType,
        ...
        DOMTransactionChain transactionChain = domDataBroker.createTransactionChain(
                new TransactionChainListener() {
                    ...
                    public void onTransactionChainFailed(TransactionChain<?, ?> transactionChain,
                        ...
                        LOG.warn("Unexpected transaction failure in transaction {}",
                                asyncTransaction.getIdentifier(), throwable);
                    }

                    @Override
                    public void onTransactionChainSuccessful(TransactionChain<?, ?> transactionChain) {
                        LOG.debug("Transaction successfully finished", transactionChain);
                    }
                });
        ...
        Futures.addCallback(future, new FutureCallback<Optional<NormalizedNode<?, ?>>>() {
            ...
            public void onSuccess(Optional<NormalizedNode<?, ?>> result) {
                if (result.isPresent()) {
                    ...
                    listener.initialize(listToMap((Collection) result.get().getValue(), underlayTopologyId));
                }
            }
