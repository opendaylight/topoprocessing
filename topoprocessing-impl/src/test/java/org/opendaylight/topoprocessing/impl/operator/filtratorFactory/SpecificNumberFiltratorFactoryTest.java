...
import org.opendaylight.yang.gen.v1.urn.opendaylight.topology.correlation.rev150121.correlations.grouping.correlations.correlation.filtration.filter.filter.type.body.SpecificNumberFilterType;
import org.opendaylight.yang.gen.v1.urn.opendaylight.topology.correlation.rev150121.correlations.grouping.correlations.correlation.filtration.filter.filter.type.body.specific.number.filter.type.SpecificNumberFilter;
...
public class SpecificNumberFiltratorFactoryTest {

    private SpecificNumberFiltratorFactory factory;
    private Filter filterMock;
    private YangInstanceIdentifier identifierMock;
    private SpecificNumberFilterType specificNumberFilterTypeMock;
    private FilterTypeBody filterTypeBodyMock;
    private SpecificNumberFilter specificNumberFilterMock;

    @Before
    public void before() {
        factory = new SpecificNumberFiltratorFactory();
        filterMock = mock(Filter.class);
        identifierMock = mock(YangInstanceIdentifier.class);
        specificNumberFilterTypeMock = mock(SpecificNumberFilterType.class);
        filterTypeBodyMock = mock(FilterTypeBody.class);
        specificNumberFilterMock = mock(SpecificNumberFilter.class);

        when(filterMock.getFilterTypeBody()).thenReturn(specificNumberFilterTypeMock);
        when(specificNumberFilterTypeMock.getSpecificNumberFilter()).thenReturn(specificNumberFilterMock);
        when(specificNumberFilterMock.getSpecificNumber()).thenReturn(5);
    }
