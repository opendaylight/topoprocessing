...
public class TopologyRequestListenerTest {
    ...
    public void testCreateCorrectNode() {
        ...
        MapEntryNode node = ImmutableNodes.mapEntryBuilder(Topology.QNAME, TopologyQNames.TOPOLOGY_ID_QNAME, TOPO_NAME)
                .withChild(ImmutableNodes.leafNode(TopologyQNames.TOPOLOGY_ID_QNAME, TOPO_NAME)).build();
    ...
    public void testCreateWrongNode1() {
        ...
        MapNode node = ImmutableNodes.mapNodeBuilder(Topology.QNAME).withChild(ImmutableNodes.mapEntryBuilder(
                Topology.QNAME, TopologyQNames.TOPOLOGY_ID_QNAME, TOPO_NAME)
                .withChild(ImmutableNodes.leafNode(TopologyQNames.TOPOLOGY_ID_QNAME, TOPO_NAME)).build()).build();
        Map<YangInstanceIdentifier, NormalizedNode<?, ?>> map = new HashMap<>();
        map.put(yiid, node);
        Mockito.when(mockChange.getCreatedData()).thenReturn(map);
