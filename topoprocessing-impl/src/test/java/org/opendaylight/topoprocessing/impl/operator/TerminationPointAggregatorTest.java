...
public class TerminationPointAggregatorTest {
    ...
    private static final QName I2RS_TERMINATION_POINT_QNAME = org.opendaylight.yang.gen.v1.urn.ietf.params.xml.ns.yang.ietf.network.topology.rev150608.network.node.TerminationPoint.QNAME;
    ...
    public void testUpdateNodeDifferentTPsNt() {
        ...
        for (MapEntryNode mapEntryNode : mapEntryNodes) {
            ...
            /*
             * switch ((String) tpRef.getValue()) { case "tp:1": case "tp:2":
             * break; case TP_ID5: Optional<DataContainerChild<? extends
             * PathArgument, ?>> tpRefs = mapEntryNode.getChild(new
             * NodeIdentifier(TopologyQNames.TP_REF)); Assert.assertTrue(
             * "Termination Point 3 should not contains TP-REFS",
             * tpRefs.isPresent()); break; default: Assert.fail(
             * "TP Node should not contain other TPs" + tpRef.get().getValue());
             * }
             */
        }
    }
