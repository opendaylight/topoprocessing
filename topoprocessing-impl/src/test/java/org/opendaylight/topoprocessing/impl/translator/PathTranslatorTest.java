...
public class PathTranslatorTest {
    ...
    public void testUnexistingModuleName() throws URISyntaxException, ParseException, IOException, YangSyntaxErrorException {
        final YangParserImpl parser = new YangParserImpl();
        List<String> modules = new ArrayList<String>();
        modules.add("/ietf-inet-types.yang");
        modules.add("/network-topology.yang");
        SchemaContext schemaContext2 = parser.parseSources(Collections2.transform(modules, new Function<String, ByteSource>() {
            @Override
            public ByteSource apply(final String input) {
                return new ByteSource() {
                    @Override
