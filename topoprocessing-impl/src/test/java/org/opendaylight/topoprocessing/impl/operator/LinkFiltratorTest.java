...
public class LinkFiltratorTest {
    ...
    public void testCreateChanges() {
        ...
        map.put(0, nodeCreator.createLeafNodeWithIpAddress("192.168.1.1"));
        filtrator.processCreatedChanges(nodeCreator.createNodeIdYiid(nodeId), new UnderlayItem(null, map,
                TOPOLOGY_NAME, nodeId, CorrelationItemEnum.Node), TOPOLOGY_NAME);

        nodeId = "node:2";
        map.put(0, nodeCreator.createLeafNodeWithIpAddress("192.168.1.2"));
        filtrator.processCreatedChanges(nodeCreator.createNodeIdYiid(nodeId), new UnderlayItem(null, map,
                TOPOLOGY_NAME, nodeId, CorrelationItemEnum.Node), TOPOLOGY_NAME);

        nodeId = "node:3";
        NormalizedNode filterOutNode = nodeCreator.createLeafNodeWithIpAddress("192.168.1.3");
        map.put(0, filterOutNode);
        ...
        map.put(0, linkCreator.createLeafNodeWithIpAddress("192.168.1.1"));
        filtrator.processCreatedChanges(linkCreator.createNodeIdYiid(linkId), new UnderlayItem(null, map,
                TOPOLOGY_NAME, linkId, CorrelationItemEnum.Link), TOPOLOGY_NAME);

        linkId = "link:2";
        map.put(0, linkCreator.createLeafNodeWithIpAddress("192.168.1.2"));
        filtrator.processCreatedChanges(linkCreator.createNodeIdYiid(linkId), new UnderlayItem(null, map,
                TOPOLOGY_NAME, linkId, CorrelationItemEnum.Link), TOPOLOGY_NAME);

        linkId = "link:3";
        NormalizedNode filterOutLink = linkCreator.createLeafNodeWithIpAddress("192.168.1.3");
        map.put(0, filterOutLink);
        ...
        linkId = "link:4";
        map.put(0, linkCreator.createLeafNodeWithIpAddress("192.168.1.4"));
        filtrator.processUpdatedChanges(linkCreator.createNodeIdYiid(linkId), new UnderlayItem(null, map,
                TOPOLOGY_NAME, linkId, CorrelationItemEnum.Link), TOPOLOGY_NAME);
        Mockito.verify(mockTopologyManager, Mockito.times(6)).addOverlayItem(Matchers.any());

        // update existing link
        linkId = "link:4";
        map.put(0, linkCreator.createLeafNodeWithIpAddress("192.168.1.14"));
        filtrator.processUpdatedChanges(linkCreator.createNodeIdYiid(linkId), new UnderlayItem(null, map,
                TOPOLOGY_NAME, linkId, CorrelationItemEnum.Link), TOPOLOGY_NAME);
        ...
        nodeId = "node:1";
        map.put(0, linkCreator.createLeafNodeWithIpAddress("192.168.1.14"));
        filtrator.processUpdatedChanges(linkCreator.createNodeIdYiid(nodeId), new UnderlayItem(null, map,
                TOPOLOGY_NAME, nodeId, CorrelationItemEnum.Node), TOPOLOGY_NAME);
        Mockito.verify(mockTopologyManager).updateOverlayItem(Matchers.any());
