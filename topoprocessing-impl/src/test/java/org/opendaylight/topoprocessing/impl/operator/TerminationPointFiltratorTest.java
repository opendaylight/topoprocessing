...
public class TerminationPointFiltratorTest {
    ...
    public void setUp() {
        ...
        RangeNumberFiltrator ntFilter = new RangeNumberFiltrator(MIN, MAX, pathForNTFiltrator);
        ...
        RangeNumberFiltrator inventoryFilter = new RangeNumberFiltrator(MIN, MAX, pathForINVFiltrator);
    ...
    public void testProcessCreatedChangesOnInventoryModel(){
        ...
        //create INV node
        LeafNode<String> conValLeaf1 = ImmutableNodes.leafNode(INVENTORY_NUMBER_VALUE_QNAME, value1);
        LeafNode<String> conValLeaf2 = ImmutableNodes.leafNode(INVENTORY_NUMBER_VALUE_QNAME, value2);

        MapEntryNode nodeConnector1 = ImmutableNodes.mapEntryBuilder(NodeConnector.QNAME,
                TopologyQNames.NODE_CONNECTOR_ID_QNAME, ncId1).withChild(conValLeaf1).build();
        MapEntryNode nodeConnector2 = ImmutableNodes.mapEntryBuilder(NodeConnector.QNAME,
                TopologyQNames.NODE_CONNECTOR_ID_QNAME, ncId2).withChild(conValLeaf2).build();
        ...
        LeafNode<String> nodeConnectorRef1 = ImmutableNodes.leafNode(
                TopologyQNames.INVENTORY_NODE_CONNECTOR_REF_QNAME, ncId1);
        LeafNode<String> nodeConnectorRef2 = ImmutableNodes.leafNode(
                TopologyQNames.INVENTORY_NODE_CONNECTOR_REF_QNAME, ncId2);
        ...
        ImmutableLeafSetEntryNodeBuilder<String> lsenBuilder = new ImmutableLeafSetEntryNodeBuilder<>();
        LeafSetEntryNode<String> lsen = lsenBuilder.withNodeIdentifier(leafSetEntry).withValue(tpRefPath).build();

        ListNodeBuilder<String, LeafSetEntryNode<String>> lnBuilder = ImmutableLeafSetNodeBuilder.create();

        NodeIdentifier nid = NodeIdentifier.create(TopologyQNames.TP_REF);
        LeafSetNode<String> leafSetNode = lnBuilder.withNodeIdentifier(nid).withChild(lsen).build();
